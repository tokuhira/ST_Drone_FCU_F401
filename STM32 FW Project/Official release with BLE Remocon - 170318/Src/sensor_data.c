...
{
    SensorAxes_t acc_temp_int16, gyro_temp_int16;       /* Data Type int16_t */
    SensorAxes_t mag_temp_int16 = { 0, 0, 0 };          /* Data Type int16_t */

    // Read data is in mg unit
    BSP_ACCELERO_Get_Axes(ACC_handle, &acc_temp_int16);
#ifdef FCU_UPSIDE_DOWN
    acc_temp_int16.AXIS_Y *= -1;
    acc_temp_int16.AXIS_Z *= -1;
#endif

    // Read data is in mdps unit
    BSP_GYRO_Get_Axes(GYR_handle, &gyro_temp_int16);
#ifdef FCU_UPSIDE_DOWN
    gyro_temp_int16.AXIS_Y *= -1;
    gyro_temp_int16.AXIS_Z *= -1;
#endif

#if USE_MAG_SENSOR == 1
    // Read data is in mgauss unit
    BSP_MAGNETO_Get_Axes(MAG_handle, &mag_temp_int16);
#ifdef FCU_UPSIDE_DOWN
    mag_temp_int16.AXIS_Y *= -1;
    mag_temp_int16.AXIS_Z *= -1;
#endif
#endif

#if USE_PRESSURE_SENSOR == 1
    // Read data is in hPa unit
    ...
    // convert acc
    acc->AXIS_X = (int32_t)acc_temp_int16.AXIS_Y;
    acc->AXIS_Y = -(int32_t)acc_temp_int16.AXIS_X;
    acc->AXIS_Z = (int32_t)acc_temp_int16.AXIS_Z;
    // convert gyro
    gyro->AXIS_X = (int32_t)gyro_temp_int16.AXIS_Y;
    gyro->AXIS_Y = -(int32_t)gyro_temp_int16.AXIS_X;
    gyro->AXIS_Z = (int32_t)gyro_temp_int16.AXIS_Z;
    // convert mag
    mag->AXIS_X = (int32_t)mag_temp_int16.AXIS_Y;
    mag->AXIS_Y = -(int32_t)mag_temp_int16.AXIS_X;
    mag->AXIS_Z = (int32_t)mag_temp_int16.AXIS_Z;
#elif COORDINATE_SYSTEM == 2
    // copy acc
    acc->AXIS_X = (int32_t)acc_temp_int16.AXIS_X;
    acc->AXIS_Y = (int32_t)acc_temp_int16.AXIS_Y;
    acc->AXIS_Z = (int32_t)acc_temp_int16.AXIS_Z;
    // copy gyro
    gyro->AXIS_X = (int32_t)gyro_temp_int16.AXIS_X;
    gyro->AXIS_Y = (int32_t)gyro_temp_int16.AXIS_Y;
    gyro->AXIS_Z = (int32_t)gyro_temp_int16.AXIS_Z;
    // copy mag
    ...
    mag->AXIS_Z = (int32_t)mag_temp_int16.AXIS_Z;
#else
#error Invalid COORDINATE_SYSTEM
#endif
}
